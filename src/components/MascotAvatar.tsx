...
export const MascotAvatar: React.FC<MascotAvatarProps> = ({
  ...
  const ids = {
    ...
    glow: `glow-${uid}`,
    skinBase: `skinBase-${uid}`,
    metalGradient: `metalGradient-${uid}`,
    orbGradient: `orbGradient-${uid}`,
    hairGradient: `hairGradient-${uid}`,
  };
  ...
  const clothing: Record<string, React.ReactNode> = {
    ...
    raincoat: (
      <g filter={`url(#${ids.softShadow})`}>
        ...
        <circle cx="85" cy="165" r="3" fill="#32D74B" filter={`url(#${ids.glow})`} />
  ...
  const accessories: Record<string, React.ReactNode> = {
    ...
    antenna: (
      <g>
        <rect x="98" y="25" width="4" height="25" fill={`url(#${ids.metalGradient})`} />
        <circle
          cx="100"
          cy="18"
          r="9"
          fill={`url(#${ids.orbGradient})`}
          filter={`url(#${ids.glow})`}
        />
    ...
    anon: (
      <g filter={`url(#${ids.softShadow})`}>
        ...
        <path d="M60,90 H140" stroke="#0A84FF" strokeWidth="2" filter={`url(#${ids.glow})`} />
      </g>
    ),
  };
  ...
  return (
    ...
    >
      <defs>
        ...
        <radialGradient id={ids.skinBase} cx="50%" cy="40%" r="60%">
          <stop offset="0%" stopColor="white" stopOpacity="0.4" />
          <stop offset="60%" stopColor={bodyColor} stopOpacity="1" />
          <stop offset="100%" stopColor="black" stopOpacity="0.15" />
        </radialGradient>
        <linearGradient id={ids.metalGradient} x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="#8E8E93" />
          <stop offset="50%" stopColor="#D1D1D6" />
          <stop offset="100%" stopColor="#8E8E93" />
        </linearGradient>
        <radialGradient id={ids.orbGradient} cx="30%" cy="30%" r="70%">
          <stop offset="0%" stopColor="#FF9F0A" />
          <stop offset="100%" stopColor="#FF375F" />
        </radialGradient>
        <linearGradient id={ids.hairGradient} x1="0%" y1="0%" x2="0%" y2="100%">
